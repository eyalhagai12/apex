...
# Regenerate sqlc query code (Postgres must be running and migrations applied)
...
**sqlc:** query execution in `marketdata/internal/storage/` and `strategy/internal/storage/` is generated by [sqlc](https://sqlc.dev) (also a Go tool dependency, `go tool sqlc ...`) rather than hand-written `database/sql` calls. Each module's `internal/storage/queries/*.sql` files are the source of truth; `sqlc.yaml` at the repo root points at them plus `migrations/` (for schema) and a live local Postgres (for resolving TimescaleDB-specific functions like `time_bucket`/`first`/`last` that aren't in sqlc's static catalog), and generates a `sqlcgen` package per module. The generated code (committed, not gitignored) is only ever called from `storage.go`/`store.go`, which keep their existing repo method signatures and convert between `sqlcgen` row/param types and the `domain` package's types — sqlc is purely a type-safe scan/exec code generator here, not a new architectural layer. Regenerate with `go tool sqlc generate` (or `make sqlc`) after changing a query file or the schema.
